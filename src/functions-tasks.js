...
function getCurrentFunctionName() {
  ...
  // Usually in Node.js: stack[1] = '    at Object.getCurrentFunctionName (file.js:line:col)'
  const callerLine = stack[1];
  const match = callerLine.match(/at (?:Object\.)?(\w+)/);
...
function getArgumentsCount(funcs) {
  ...
  return funcs.map((fn) => (typeof fn === 'function' ? fn.length : 0));
}
...
function partialUsingArguments(fn, ...args1) {
  return function partialUsingArgumentsFunc(...args2) {
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function getIdGeneratorFunc() {
    const id = currentId;
    currentId += 1;
    return id;
  };
